<div id="table_data">
    <h3 style="font-weight:bold; font-size:32px;" class="mt-2">Payment</h3>
<hr>

<div class="col-12">
    <div class = "row">
        <div class="col-6">
            <a onclick="object.setUrl('/payment/form').setMethod('Get').load()" href="javascript:void(0);" id="formid" class="btn btn-md btn-success mb-4"><i class="fas fa-plus-square"></i> Create New Payment</a>
        </div>
        <div class="col-6">
        </div>
    </div>
            ...
            <tbody>
                ...
                @else
                    @foreach ($payments as $value)
                        ...
                        <td>
                        @if($value->status == 1)
                        <a onclick="object.setUrl('/payment/status/{{$value->id}}').setMethod('get').load();" href="javascript:void(0);" class="btn btn-warning">Enable</a>
                            @else
                            <a onclick="object.setUrl('/payment/status/{{$value->id}}').setMethod('get').load();" href="javascript:void(0);" class="btn btn-danger"> Disable</a>
                        @endif
                        </td>
                        <td><a onclick="object.setUrl('/payment/form/{{$value->id}}').setMethod('get').load();" href="javascript:void(0);" class="btn btn-success">Edit</a></td>
                        <td> <a onclick="object.setUrl('/paymentDelete/{{ $value->id }}').setMethod('get').load()" href="javascript:void(0)" class="btn btn-secondary">Delete</a></td>
                        </tr>
                    @endforeach
                @endif
            </tbody>
